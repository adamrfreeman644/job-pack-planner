(() => {
 ...
 function enhance(){
  timeline.querySelectorAll('.event:not(.home)').forEach(event=>{const save=event.querySelector('[data-save]');if(!save)return;const id=Number(save.dataset.save),title=event.querySelector('.event-title');event.dataset.routeId=id;event.draggable=true;event.querySelector('.route-leg')?.remove();
   if(!title.querySelector('.reorder-controls')){const controls=document.createElement('span');controls.className='reorder-controls';controls.innerHTML=`<button type="button" data-move="-1" aria-label="Move job earlier">↑</button><button type="button" data-move="1" aria-label="Move job later">↓</button><button type="button" data-lock aria-label="Lock job position" aria-pressed="${locks.has(id)}">${locks.has(id)?'🔒':'🔓'}</button><span class="drag-handle" title="Drag to reorder">⠿</span>`;title.append(controls)}
