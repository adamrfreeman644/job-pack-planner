(() => {
  ...
  function show(data){home.value=data.home_address||'';key.value='';key.placeholder=data.has_api_key?'Saved — leave blank to keep it':'Paste Google Routes API key';status.textContent=data.has_api_key?'Google route optimisation is configured.':'Add an API key to enable automatic optimisation.';clear.hidden=!data.has_api_key}
